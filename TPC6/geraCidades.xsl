...
:nome "<xsl:value-of select="designacao"/>";
:populacao "100000"
            <xsl:for-each select=".//freguesia">
                :C<xsl:value-of select="generate-id($c)"/> :temFreguesia :F<xsl:value-of select="generate-id(.)"/>.              
            </xsl:for-each>
            <xsl:call-template name="linha"/>
        </xsl:for-each>

### Freguesias
        <xsl:for-each select="//freguesia[not(designacao=preceding::designacao)]">
###  http://prc2018.di.uminho.pt/mapa#F<xsl:value-of select="generate-id(.)"/>
:F<xsl:value-of select="generate-id(.)"/> rdf:type owl:NamedIndividual ,
:Freguesia ;
:nome "<xsl:value-of select="designacao"/>".
            <xsl:call-template name="linha"/>
        </xsl:for-each>
